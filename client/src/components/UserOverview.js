...
function UserOverview({ currentUser }) {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const response = await baseURL.get("/properties", {
          ...
          {properties.length === 0 ? (
            <div className="border-8 border-none shadow-2xl opacity-70 rounded-3xl flex items-center justify-center mt-32 m-40 bg-blue-300">
              <Link to="/add-property">
                <h1 className="m-10 text-3xl hover:text-blue-600 transition duration-300 animate-pulse">
            ...
            <div>
              <div>
                <h1 className="flex items-center justify-center text-3xl mt-5">
                  Overview
                </h1>
              </div>
